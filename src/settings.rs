...
pub fn load_settings() -> Result<Settings, &'static str> {
    let settings_file = File::open(SETTINGS_PATH).map_err(
        |_| "Failed to locate settings file."
    )?;
    let reader = BufReader::new(settings_file);
    let settings = serde_json::from_reader(reader).map_err(
        |_| "Failed to deserialize settings file contents."
    )?;
    Ok(settings)
}